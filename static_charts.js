(function () {
    ...
    d3.csv(FILE).then(function (data) {
      // coerce year columns to numbers
    ...
    function renderHeatmap(data) {
      ...
      const filtered = data.filter(
        d =>
          ...
          continents.includes(d.Country) &&
          industriesOrder.includes(d.Industry)
      );
  
      // aggregate by continent × industry
      const byCI = d3.rollup(
        filtered,
        v => d3.sum(v, d => d[year]),
        d => d.Country,
        d => d.Industry
      ...
      continents.forEach(region => {
        industriesOrder.forEach(ind => {
          const v = (byCI.get(region) && byCI.get(region).get(ind)) || 0;
